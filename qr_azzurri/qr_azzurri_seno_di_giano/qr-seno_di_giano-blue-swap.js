(function () {
  var data = {
    parent: {
      id: "seno_di_giano",
      label: "Seno di Giano",
      lat: 44.403319856181746,
      lng: 8.931439046823426
    },
    children: [
      {
        id: "campana",
        label: "Scoglio Campana",
        lat: 44.40338522320385,
        lng: 8.931322940778454,
        descr: "Scoglio Campana sotto le Mura delle Grazie.",
        media: {
          oggi: "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_campana/qr_azzurri_seno_di_giano_campana_oggi.mp4",
          ieri: [
            "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_campana/qr_azzurri_seno_di_giano_campana_ieri_1.mp4"
          ]
        }
      },
      {
        id: "mura_grazie",
        label: "Mura delle Grazie",
        lat: 44.40382060888646,
        lng: 8.930566983445246,
        descr: "Mura delle Grazie.",
        media: {
          oggi: "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_mura_grazie/qr_azzurri_seno_di_giano_mura_grazie_oggi.mp4",
          ieri: [
            "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_mura_grazie/qr_azzurri_seno_di_giano_mura_grazie_ieri_1.mp4"
          ]
        }
      },
      {
        id: "piazzetta",
        label: "Piazzetta",
        lat: 44.40287052449395,
        lng: 8.932006353117693,
        descr: "Piazzetta in fondo a Via Madre di Dio",
        media: {
          oggi: "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_piazzetta/qr_azzurri_seno_di_giano_piazzetta_oggi.mp4",
          ieri: [
            "qr_azzurri/qr_azzurri_seno_di_giano/qr_azzurri_seno_di_giano_piazzetta/qr_azzurri_seno_di_giano_piazzetta_ieri_1.mp4"
          ]
        }
      }
    ]
  };

  // Anti-doppione: se questo sorgente è già stato aggiunto, esci.
  var key = "__qr_src_added__" + (data.parent && data.parent.id ? data.parent.id : "seno_di_giano");
  if (window[key]) return;
  window[key] = true;

  function publish(){
    try{
      if (window.__qrAddSource) {
        window.__qrAddSource(data.parent, data.children);
        return true;
      }
      window.__QR_SOURCES = window.__QR_SOURCES || [];
      window.__QR_SOURCES.push({ parent: data.parent, children: data.children });
      return true;
    }catch(e){
      return false;
    }
  }

  function build(){
    try { window.__qrBuildAll && window.__qrBuildAll(); } catch(_) {}
  }

  if(!publish()){
    var tries = 0;
    var t = setInterval(function(){
      if(publish() || (++tries > 60)){
        clearInterval(t);
        build();
      }
    }, 100);
  }else{
    build();
  }


  whenMapReady(function () {
    var parent = data.parent, kids = data.children || [];
    var chk = document.getElementById("chk-qr-seno-di-giano");
    if (!chk) return;

    if (!map.getPane("pane-seno_di_giano-blue")) {
      map.createPane("pane-seno_di_giano-blue");
      map.getPane("pane-seno_di_giano-blue").style.zIndex = 650;
    }
    if (!map.getPane("pane-seno_di_giano-azzurri")) {
      map.createPane("pane-seno_di_giano-azzurri");
      map.getPane("pane-seno_di_giano-azzurri").style.zIndex = 651;
    }

    // Fallback: se blueIcon non esiste, lo creiamo.
    var blueIconLocal = window.blueIcon;
    if (!blueIconLocal) {
      blueIconLocal = L.divIcon({
        className: "qr-blue-parent",
        iconSize: [16, 14],
        iconAnchor: [8, 14],
        html: '<div class="qr-triangle"></div>'
      });
      window.blueIcon = blueIconLocal;
    }

    var azzIcon = L.icon({
      iconUrl: "qr_azzurri/marker-azzurro-qr-notch-24.svg",
      iconSize: [24, 26],
      iconAnchor: [12, 26],
      className: "qr-azzurro-icon"
    });

    var blue = L.marker([parent.lat, parent.lng], {
      pane: "pane-seno_di_giano-blue",
      icon: blueIconLocal,
      title: parent.label
    });

    try { blue.off("click"); } catch (e) {}
    if (blue.bindTooltip) {
      blue.bindTooltip(parent.label || "", {
        permanent: false,
        direction: "right",
        offset: [8, 0],
        className: "qr-tooltip"
      });
    }
    blue.on("click", function () {
      if (window.__qrOpenChildPanel) {
        window.__qrOpenChildPanel(
          parent.label || "",
          "Zooma per vedere i punti di Seno di Giano.",
          { oggi: null, ieri: [] }
        );
      }
    });

    var group = L.layerGroup();
    (kids || []).forEach(function (c, idx) {
      var lat = c.lat, lng = c.lng;

      if (
        Math.abs((lat || 0) - (parent.lat || 0)) < 1e-7 &&
        Math.abs((lng || 0) - (parent.lng || 0)) < 1e-7
      ) {
        var offs = [[0, 0.00035], [0.0003, 0], [-0.00028, -0.00022], [0.00022, -0.00025]];
        var d = offs[idx % offs.length];
        lat += d[0]; lng += d[1];
      }

      var m = L.marker([lat, lng], {
        pane: "pane-seno_di_giano-azzurri",
        icon: azzIcon,
        title: c.label
      });

      if (m.bindTooltip) {
        m.bindTooltip(c.label || "", {
          permanent: false,
          direction: "right",
          offset: [8, 0],
          className: "qr-tooltip"
        });
      }

      m.on("click", function () {
        if (window.__qrOpenChildPanel) {
          window.__qrOpenChildPanel(
            c.label || "",
            c.descr || "",
            c.media || { oggi: null, ieri: [] }
          );
        }
      });

      group.addLayer(m);
    });

    function showBlue() { try { group.removeFrom(map); } catch (e) {} try { blue.addTo(map); } catch (e) {} }
    function showKids() { try { blue.removeFrom(map); } catch (e) {} try { group.addTo(map); } catch (e) {} }
    function hideAll()  { try { blue.removeFrom(map); } catch (e) {} try { group.removeFrom(map); } catch (e) {} }

    var THRESH = 0;
    function update() {
      var on = !!(chk && chk.checked);
      if (!on) { hideAll(); return; }
      var z = map.getZoom ? map.getZoom() : 0;
      showKids();
    }

    if (!chk.__wiredSwap_seno_di_giano) {
      chk.addEventListener("change", update);
      chk.__wiredSwap_seno_di_giano = true;
    }
    map.on("zoomend", update);
    update();
  });
})();
