...
(function(){
  ...
  var overlay=null,body=null,notice=null,lastFocused=null,detailOverlay=null,detailBody=null;
  ...
  function createDetail(){
    ...
    detailOverlay=el('div','subscribers-admin-detail-overlay');detailOverlay.setAttribute('aria-hidden','true');
    var card=el('section','subscribers-admin-detail');card.setAttribute('role','dialog');card.setAttribute('aria-modal','true');card.setAttribute('aria-labelledby','subscribers-admin-detail-title');
    var head=el('div','subscribers-admin-detail-head');var title=el('h3','', 'Scheda utente');title.id='subscribers-admin-detail-title';var close=button('subscribers-admin-detail-close','×');close.setAttribute('aria-label','Chiudi scheda utente');head.appendChild(title);head.appendChild(close);
    detailBody=el('div','subscribers-admin-detail-body');card.appendChild(head);card.appendChild(detailBody);detailOverlay.appendChild(card);document.body.appendChild(detailOverlay);
    close.addEventListener('click',closeDetail);detailOverlay.addEventListener('click',function(event){if(event.target===detailOverlay)closeDetail();});
  }
  ...
  function openDetail(user){
    if(!detailOverlay||!detailBody)return;detailBody.textContent='';
    ...
    detailOverlay.classList.add('is-open');detailOverlay.setAttribute('aria-hidden','false');
  }
  function closeDetail(){if(!detailOverlay)return;detailOverlay.classList.remove('is-open');detailOverlay.setAttribute('aria-hidden','true');}
  ...
  function render(){
    ...
    var top=el('div','subscribers-admin-list-head');var titleWrap=el('div');titleWrap.appendChild(el('h3','','Elenco iscritti e abbonati'));titleWrap.appendChild(el('p','','Clicca su una riga per aprire la scheda dell’utente.'));top.appendChild(titleWrap);var refresh=button('subscribers-admin-secondary','Aggiorna elenco');refresh.addEventListener('click',function(){load(refresh);});top.appendChild(refresh);card.appendChild(top);
    ...
    var wrap=el('div','subscribers-admin-table-wrap');var table=el('table','subscribers-admin-table');var thead=document.createElement('thead');var hr=document.createElement('tr');['Email','Stato','Tipo','Piano','Scadenza','Iscritto'].forEach(function(label){var th=document.createElement('th');th.textContent=label;hr.appendChild(th);});thead.appendChild(hr);var tb=document.createElement('tbody');table.appendChild(thead);table.appendChild(tb);wrap.appendChild(table);card.appendChild(wrap);body.appendChild(card);
    ...
    function refreshRows(){
      ...
      data.users.forEach(function(user){
        ...
        var tr=document.createElement('tr');tr.tabIndex=0;tr.setAttribute('role','button');tr.setAttribute('aria-label','Apri scheda di '+(user.email||'utente'));
        ...
        tr.addEventListener('click',function(){openDetail(user);});tr.addEventListener('keydown',function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();openDetail(user);}});tb.appendChild(tr);
      });
  ...
  document.addEventListener('keydown',function(event){if(event.key!=='Escape')return;if(detailOverlay&&detailOverlay.classList.contains('is-open')){closeDetail();return;}if(overlay&&overlay.classList.contains('is-open'))closePanel();});
